...
class MyPatientSeparateThroughAccountingProfileResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $role = $this->doctorUser->roles->where('roleable_id', $this->patient_id)->where('roleable_type', 'App\Models\Patient')->first();
        if ($role) {
            ...
            if ($role->sub_role == DoctorRoleForPatient::DoctorWithoutApprove) {
                ...
                return [
                    ...
                    'image' => $originalPatient->doctorImage != null ?
                        DoctorImageResource::collection($this->patient->doctorImage->where('doctor_id', $this->doctorUser->doctor->id))
                        : "no image",
                    'medical_info' => new MedicalInformationResource($this->patient->allMedicalInformation->where('doctor_id', $this->doctorUser->doctor->id)),
                ];
            }
        }

...
class DoctorInfoController extends Controller
{
    ...
    public function showMyPatient(Office $office)
    {
        ...
        if ($doctor) {
            if ($office->type == OfficeType::Combined) {
                ...
                $accounts = AccountingProfile::where(['doctor_id' => $ownerDoctor->id, 'office_id' => $office->id, 'type' => AccountingProfileType::PatientAccount])->with(['office', 'office.owner', 'patient'])->get();
                ...
                $accounts = AccountingProfile::where(['doctor_id' => auth()->user()->doctor->id, 'office_id' => $office->id, 'type' => AccountingProfileType::PatientAccount])->with(['office', 'office.owner', 'patient'])->get();
    ...
    public function activePatient(Office $office)
    {
        ...
        if ($doctor) {
            ...
            $patients = Patient::whereHas('appointments', function (Builder $query) {
                $query->where('taken_date', '>', now()->subDays(30)->endOfDay())->where('doctor_id', auth()->user()->doctor->id);
            })->get();
            return $patients;

...
class DoctorInfoController extends Controller
{
    ...
    public function showMyPatient(Office $office)
    {
        ...
        if ($doctor) {
            if ($office->type == OfficeType::Combined) {
                ...
                $accounts = AccountingProfile::where(['doctor_id' => $ownerDoctor->id, 'office_id' => $office->id, 'type' => AccountingProfileType::PatientAccount])->with(['office', 'office.owner', 'office.owner.user', 'patient', 'patient.info', 'patient.doctorImage'])->get();
                // if ($accounts != []) {
                ...
                $accounts = AccountingProfile::where(['doctor_id' => auth()->user()->doctor->id, 'office_id' => $office->id, 'type' => AccountingProfileType::PatientAccount])->with(['office', 'patient', 'patient.info', 'patient.doctorImage'])->get();
                // return $accounts;
                // if ($accounts != []) {
    ...
    public function myInvoiceReceipt(Office $office)
    {
        ...
        return InvoiceReceiptsResource::collection($doctor->invoiceReceipts()
            ...
            return new InvoiceReceiptsResource($accountProfile, $user);
        });
    }
